...
// Bell path reused in nav + mobile header
    ...
    <div className="flex min-h-screen" style={{ background: 'var(--color-bg)' }}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        >
          ...
          >
            <span style={{ display: 'block', height: '1.5px', width: '18px', borderRadius: '2px', background: 'var(--color-muted)', transition: 'all 280ms ease', transform: sideOpen ? 'translateY(6.5px) rotate(45deg)' : 'none' }} />
            <span style={{ display: 'block', height: '1.5px', width: '18px', borderRadius: '2px', background: 'var(--color-muted)', transition: 'all 280ms ease', opacity: sideOpen ? 0 : 1, transform: sideOpen ? 'scaleX(0)' : 'scaleX(1)' }} />
            <span style={{ display: 'block', height: '1.5px', width: '18px', borderRadius: '2px', background: 'var(--color-muted)', transition: 'all 280ms ease', transform: sideOpen ? 'translateY(-6.5px) rotate(-45deg)' : 'none' }} />
          </button>
          ...
          {/* Bell icon with count badge — replaces plain avatar */}
          <Link href={`/dashboard/${role}/notifications`}
            className="relative flex items-center justify-center w-9 h-9 rounded-lg"
            style={{ background: 'rgba(255,255,255,0.05)', color: 'var(--color-muted)', textDecoration: 'none' }}>
            <svg width="18" height="18" fill="none" viewBox="0 0 24 24"
              stroke="currentColor" strokeWidth={1.75}>
              <path strokeLinecap="round" strokeLinejoin="round" d={BELL_D} />
            </svg>
            {/* Badge sits top-right of the bell button */}
            <span className="absolute" style={{ top: 2, right: 2 }}>
              <NotificationBadge />
            </span>
          </Link>
        </header>
